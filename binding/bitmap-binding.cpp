...
RB_METHOD_GUARD(bitmapBlendBlt) {
    ...
    VALUE srcObj;
    VALUE srcRectObj;
    int blend_type = Bitmap::BLEND_NORMAL;
    int opacity = 255;

    Bitmap *src;
    Rect *srcRect;

    rb_get_args(argc, argv, "iioo|ii", &x, &y, &srcObj, &srcRectObj,
                &blend_type, &opacity RB_ARG_END);

    src = getPrivateDataCheck<Bitmap>(srcObj, BitmapType);
    if (src) {
        srcRect = getPrivateDataCheck<Rect>(srcRectObj, RectType);

        if (blend_type < Bitmap::BLEND_NORMAL || blend_type > Bitmap::BLEND_OVERLAY)
            blend_type = Bitmap::BLEND_NORMAL;
        if (opacity < 0) opacity = 0;
        if (opacity > 255) opacity = 255;

        GFX_GUARD_EXC(
            b->blendBlt(x, y, *src, srcRect->toIntRect(), blend_type, opacity);
        );
    }
...
void bitmapBindingInit() {
    ...
    _rb_define_method(klass, "tktk_blend_blt", bitmapBlendBlt);

    // Blend mode constants
    rb_define_const(klass, "BLEND_NORMAL", INT2FIX(Bitmap::BLEND_NORMAL));
    rb_define_const(klass, "BLEND_ADD",    INT2FIX(Bitmap::BLEND_ADD));
    rb_define_const(klass, "BLEND_SUB",    INT2FIX(Bitmap::BLEND_SUB));
    rb_define_const(klass, "BLEND_MUL",    INT2FIX(Bitmap::BLEND_MUL));
    rb_define_const(klass, "BLEND_DODGE",  INT2FIX(Bitmap::BLEND_DODGE));
    rb_define_const(klass, "BLEND_BURN",   INT2FIX(Bitmap::BLEND_BURN));
    rb_define_const(klass, "BLEND_SCREEN", INT2FIX(Bitmap::BLEND_SCREEN));
    rb_define_const(klass, "BLEND_OVERLAY",INT2FIX(Bitmap::BLEND_OVERLAY));
